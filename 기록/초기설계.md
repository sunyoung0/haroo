...
diary_member(공유 다이어리 멤버) - group_id(FK), user_id(FK), role(owner/member)

diary(일기) - id(PK), user_id(FK- 작성자), group_id(FK - 어느 다이어리인지), title, content, image_url, feeling_type(감정), created_at

diary_comment(댓글) - id(PK), diary_id(어느 다이어리 댓글인지), content, created_by(FK - user.id, 댓글 작성자), created_at
...
- **HTTP메소드:** GET -**URL** `/groups/{user_id}`
...
### 2-1. 다이어리 조회(날짜별 / 회원별 필터링)

- **목적:** 일기 내용 날짜별로 조회/ 캘린더 날짜 눌렀을 시 조회
- **HTTP메소드:** GET
- **URL:** `/diary/{diary_id}/diaries`
