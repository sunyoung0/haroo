...
## 조회 시 빈 값이 나옴
- 에러 : 멤버 조회 시 빈 값이 나옴 groupId가 null
- 해결 : controller에 @PathVariable Long groupId PathVariable을 붙이지 않아서 groupId를 받을 수 없었음

## 중복 매핑

- 에러 메시지 : Caused by: java.lang.IllegalStateException: Ambiguous mapping. Cannot map 'diaryGroupController' method

- 이유 : `@DeleteMapping("/{groupId}")` deleteMapping의 URL을 동일하게 작성했어서 오류 발생

- 해결 : URL 수정

## 다이어리 그룹 삭제 시 외래 키(Foreign Key) 제약 조건 위반 에러

- 에러 : DiaryGroup(부모)을 삭제하려고 하는데, 그 그룹 안에 작성된 **Diary(자식, 일기 데이터)**들이 여전히 그 그룹의 ID를 참조하고 있기 때문

- 해결 : JPA의 Cascade 활용
   DiaryGroup 엔티티에서 Diary와의 연관관계 설정에 cascade = CascadeType.ALL과 orphanRemoval = true를 추가 -> 그룹이 삭제될 때 하위 일기들도 함께 삭제